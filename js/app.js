...
class App {
  ...
  /**
   * تحميل بيانات عينة - الحل النهائي
   */
  loadSampleData() {
    ...
    // إذا كان هناك منتجات، لا نضيف بيانات عينة
    ...
    console.log("📌 جاري إضافة بيانات عينة لأول مرة...");

    // إضافة بيانات عينة فقط إذا لم تكن هناك بيانات ولا تم المسح من قبل
    ...
    console.log("✅ تم إضافة البيانات العينة بنجاح");
  }

  /**
   * تحميل لوحة التحكم - النسخة المصححة
   */
  loadDashboard() {
    try {
  ...
  /**
   * تحديث البيانات بعد البيع أو الإلغاء - النسخة المصححة
   */
  refreshDataAfterSale() {
    ...
    // تحديث جميع الصفحات التي تعرض البيانات
    ...
    try {
      ...
      this.pageContent.innerHTML = `
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
                    <h1 style="font-size:1.5rem;">📦 إدارة المخزون</h1>
                    <button class="btn btn-primary" onclick="app.showAddProduct()" style="padding:10px 20px;background:var(--primary-color);color:white;border:none;border-radius:var(--radius);cursor:pointer;">
                        ➕ إضافة قطعة جديدة
                    </button>
                </div>
                ...
                <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:20px;">
                    <input type="text" id="filterSearch" placeholder="بحث..." oninput="app.filterInventory()" style="flex:1;min-width:150px;padding:10px 12px;border:1px solid var(--border-color);border-radius:var(--radius);background:var(--bg-color);color:var(--text-color);">
                    <select id="filterCategory" onchange="app.filterInventory()" style="padding:10px 12px;border:1px solid var(--border-color);border-radius:var(--radius);background:var(--bg-color);color:var(--text-color);">
                        <option value="">جميع الشركات</option>
                        ${this.inventory.categories.map((c) => `<option value="${c}">${c}</option>`).join("")}
                    </select>
                    <select id="filterStatus" onchange="app.filterInventory()" style="padding:10px 12px;border:1px solid var(--border-color);border-radius:var(--radius);background:var(--bg-color);color:var(--text-color);">
                    ...
                    <button class="btn btn-success" onclick="app.exportData()" style="padding:10px 20px;background:#16a34a;color:white;border:none;border-radius:var(--radius);cursor:pointer;">📤 تصدير</button>
                    <button class="btn btn-warning" onclick="app.importData()" style="padding:10px 20px;background:#f59e0b;color:white;border:none;border-radius:var(--radius);cursor:pointer;">📥 استيراد</button>
                </div>
      ...
      if (this.pageContent) {
        this.pageContent.innerHTML =
          '<p style="text-align:center;padding:40px;color:var(--text-secondary);">حدث خطأ في تحميل المخزون</p>';
      }
    }
  }
      ...
      .map(
        (p, index) => `
            <tr>
                ...
                <td style="padding:12px 16px;border-bottom:1px solid var(--border-color);">${this.getStatusLabel(p.status)}</td>
                <td style="padding:12px 16px;border-bottom:1px solid var(--border-color);">
  ...
  createProductForm(product = null) {
    ...
    form.innerHTML = `
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="productDescription" rows="3" style="width:100%;padding:10px 12px;border:1px solid var(--border-color);border-radius:var(--radius);background:var(--bg-color);color:var(--text-color);">${p.description || ""}</textarea>
            </div>
            <div class="form-group">
                <label style="display:block;margin-bottom:4px;font-weight:500;">المواصفات الفنية</label>
                <textarea class="form-control" id="productSpecifications" rows="3" style="width:100%;padding:10px 12px;border:1px solid var(--border-color);border-radius:var(--radius);background:var(--bg-color);color:var(--text-color);">${p.specifications || ""}</textarea>
            </div>
        `;
    return form;
  }
  ...
  /**
   * مسح جميع البيانات - النسخة النهائية (دالة واحدة فقط)
   */
  clearAllData() {
    this.modal.open({
      ...
      confirm: () => {
        try {
          ...
          // 2. تعيين علامة "تم المسح" لمنع إعادة البيانات العينة
    ...
    this.modal.open({
      ...
      confirm: () => {
        try {
          ...
          // 2. إزالة علامة "تم المسح" للسماح بإعادة البيانات
  ...
  /**
   * تحميل المبيعات - النسخة المصححة (مع عرض الربح)
   */
  loadSales() {
    try {
      // تحديث البيانات أولاً
  ...
  /**
   * عرض تفاصيل الفاتورة - النسخة المصححة
   */
  viewInvoice(id) {
    try {
  ...
  /**
   * إلغاء فاتورة - النسخة المصححة (مع تحديث فوري)
   */
  cancelInvoice(id) {
    this.modal.open({
                          ...
                          .map((cat) => {
                            ...
                            const percentage =
                              stats.totalItems > 0
                                ...
                                : 0;
                            return `
                                <div style="background:var(--bg-color);padding:16px;border-radius:var(--radius);text-align:center;">
  ...
  /**
   * عرض إعدادات النظام - مع إضافة زر البيانات العينة
   */
  loadSettings() {
    try {
      if (!this.pageContent) return;
  ...
  toggleDarkMode() {
    ...
    // إعادة تحميل الإعدادات إذا كنا في صفحة الإعدادات
